...
在谷歌浏览器的控制台输入**vm.text = 'hello china'**,** **浏览器显示的内容从**'hello world'变成了'hello china'**, 惊讶吧, 我们并没有操作dom哦, 只是操作了数据, 然后vue帮我们自动改变了dom. 这里在控制台去改数据只是我们还没有学习输入绑定事件, 当我学会绑定事件后, 我们就可以在自定义方法中痛快的操作数据了.
